...
import { FeedbackForm } from "@/components/modules/feedback-form";
...
export default async function ModuleCompletedPage({
  ...
  // Get completion date and existing feedback in parallel
  const [lastCompletion, existingFeedback] = await Promise.all([
    prisma.sectionCompletion.findFirst({
      where: {
        userId: ctx.user.id,
    ...
    prisma.moduleSelfAssessment.findUnique({
      where: {
    ...
    <div className="flex min-h-[60vh] items-center justify-center py-12 px-4">
      <div className="w-full max-w-lg space-y-4">
        ...
        <FeedbackForm
          ...
          existingNote={existingFeedback?.note ?? undefined}
        />
      </div>
    </div>
  );
