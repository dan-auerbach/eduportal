...
export function XpDisplay({
  ...
  if (variant === "compact") {
    return (
      ...
      >
        ...
        <span className="text-sm font-semibold tabular-nums">{totalXp.toLocaleString()}</span>
  ...
  return (
    <div data-slot="xp-display" className={cn("space-y-2", className)}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Zap className="h-5 w-5 text-yellow-500" />
          <span className="text-lg font-bold tabular-nums">{totalXp.toLocaleString()} XP</span>
        </div>
        ...
        <div className="space-y-1">
          ...
          <div className="h-2 w-full rounded-full bg-muted">
            <div
              ...
              style={{
                width: `${Math.max(5, 100 - (nextRank.xpNeeded / (totalXp + nextRank.xpNeeded)) * 100)}%`,
              }}
            />
          </div>
        </div>
